...
export const useUpdateOrderStatus = () => {
  ...
  const updateOrderStatusRequest = async (
    updatedStatus: UpdateOrderStatusRequest
  ): Promise<Order> => {
    const accessToken = await getAccessTokenSilently();

    const response = await fetch(
      `${API_BASE_URL}/api/my/restaurant/order/${updatedStatus.orderId}/status`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        ...
        body: JSON.stringify({ status: updatedStatus.status }),
      }
    );
  ...
  if (isError) {
    toast.error("Unable to update order", { duration: 2000 });
    reset();
  }
